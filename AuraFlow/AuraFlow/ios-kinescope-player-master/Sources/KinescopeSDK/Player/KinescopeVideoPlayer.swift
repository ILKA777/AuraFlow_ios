...
public protocol KinescopeVideoPlayerDelegateForManager: AnyObject {
    func player(_ player: KinescopeVideoPlayer, didUpdateTime time: Double)
}
...
public class KinescopeVideoPlayer: KinescopePlayer, KinescopePlayerBody, FullscreenStateProvider, PlayingRateSource, VideoQualitySource, SubtitlesSource {
    ...
    public weak var delegateManager: KinescopeVideoPlayerDelegateForManager?
    ...
    public var duration: TimeInterval {
        return strategy.player.durationSeconds ?? 0.0
    }

    public var currentTime: TimeInterval {
        return strategy.player.currentTime().seconds
    }
...
//    }
    ...
    func observePlaybackTime() {
            ...
            playbackObserverFactory = PlaybackTimePeriodicObserver(
                ...
                secondsPlayed: { [weak self] updatedTime in
                    ...
                    // Обновление времени через делегата
                    self.delegateManager?.player(self, didUpdateTime: updatedTime)
    ...
    public func removePlaybackTimeObserver() {
    ...
    public func addPlayerItemObserver() {
